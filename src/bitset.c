...
void writeBit(bitset array, int bitIndex, int byteOffset, int value)
{
    // we add 1 because the first byte is used to store the length of the array
    int byteIndex = bitIndex / 8 + byteOffset;
    int bitOffset = bitIndex % 8;
    array[byteIndex] = value == 0 ? ~((1 << bitOffset) | ~array[byteIndex]) : 1 << bitOffset | array[byteIndex] ;
}

uint8_t readBit(bitset array, int bitIndex, int byteOffset)
{
    // we add 1 because the first byte is used to store the length of the array
    int byteIndex = bitIndex / 8 + byteOffset;
    int bitOffset = bitIndex % 8;
